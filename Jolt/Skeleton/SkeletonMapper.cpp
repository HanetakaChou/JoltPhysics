...
{
	{
		...
		mcrt_vector<brx_motion_skeleton_joint_transform> skeleton_a_bind_pose_local_space(static_cast<size_t>(inSkeleton1->GetJointCount()));
		{
		...
		mcrt_vector<brx_motion_skeleton_joint_transform> skeleton_b_bind_pose_local_space(static_cast<size_t>(inSkeleton2->GetJointCount()));
		{
		...
		brx_animation_skeleton_mapper_create_ragdoll_mapping(
			skeleton_a,
			skeleton_b,
			[](brx_animation_skeleton const &in_skeleton_a, brx_animation_skeleton const &in_skeleton_b, uint32_t const in_skeleton_b_joint_index, void *const in_user_data_map_joint_b_to_a) -> uint32_t
			{
				mcrt_unordered_map<std::string_view, uint32_t> const &in_skeleton_a_joint_names_map = *static_cast<mcrt_unordered_map<std::string_view, uint32_t> *>(in_user_data_map_joint_b_to_a);
				mcrt_unordered_map<std::string_view, uint32_t>::const_iterator found_skeleton_a_joint = in_skeleton_a_joint_names_map.find(in_skeleton_b.get_joint_name(in_skeleton_b_joint_index));
				if (in_skeleton_a_joint_names_map.end() != found_skeleton_a_joint)
				{
					return found_skeleton_a_joint->second;
				}
				else
				{
					return BRX_MOTION_SKELETON_JOINT_INDEX_INVALID;
				}
			},
			&skeleton_a_joint_names_map,
			inLockTranslations,
			this->m_ragdoll_mapping_data);
	}
